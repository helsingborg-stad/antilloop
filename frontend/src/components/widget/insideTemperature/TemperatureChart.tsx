...
const TemperatureChart = memo(
  ({ data, size, xRange, rangeUpdated }: BarChartProps) => {
    ...
    const scaleY = scaleLinear()
      .domain([yMin < 10 ? yMin : 10, yMax > 32 ? yMax + 2 : 32])
      .range([height, 0]);
    ...
    const colorStops = [
      { color: "#FF462C", percent: 0 },
      { color: "#F6D438", percent: 20 },
      { color: "#5ADD41", percent: 45 },
          ...
          >
            ...
            >
              <m.path
                ...
                d={linePath}
                opacity={1}
                stroke={`url(#inside-temp-${timestampId})`}
                strokeOpacity="0.2"
                fill="none"
                strokeWidth={24}
                strokeLinecap="round"
