...
import { getCurrentParticles25 } from "@/utils/widgetCurrentData";
...
const Particles: FC<ParticlesProps> = ({
  ...
  const currentLevel =
    currentTestLevel !== undefined
      ? currentTestLevel
      : latestData?.data?.pm2_5_mass_concentration;
  ...
  const graphData = data
    ? data
        ?.filter(
          (item: {
            data: { date: string; pm2_5_mass_concentration: number | string };
          }) => typeof item.data.pm2_5_mass_concentration === "number"
        )
        .map(
          (item: {
            data: { date: string; pm2_5_mass_concentration: number };
          }) => ({
            x: item.data.date,
            y: Number(item.data.pm2_5_mass_concentration.toFixed(1))
          })
        )
    : null;
  ...
  const settings =
    currentLevel || currentLevel === 0
      ? getCurrentParticles25(currentLevel)
      : !isLoadingLatest
      ? {
          currentLevel: null,
          color: "",
        ...
        <div className="no-scrollbar max-h-[65vh] max-w-[560px] overflow-y-auto">
          <div className="mb-4">
            <p className="mb-4 rounded-2xl bg-blue-70 p-2">
              {t("particles.25_info.1")}
            </p>
            <p className="mb-4 rounded-2xl bg-blue-70 p-2">
              {t("particles.25_info.2")}
            </p>
            <p className="mb-4 rounded-2xl bg-blue-70 p-2">
              {t("particles.25_info.3")}
            </p>
            <p className="rounded-2xl bg-blue-70 p-2">
              {t("particles.25_info.4")}
            </p>
          </div>
        </div>
      </Modal>
