...
const TemperatureChart = memo(
  ({ data, size, xRange, rangeUpdated }: BarChartProps) => {
    ...
    const scaleY = scaleLinear()
      .domain([yMin < -25 ? yMin : -25, yMax > 35 ? yMax : 35])
      .range([height, 0]);
    ...
    const colorStops = [
      { color: "#A43A67", percent: 0 },
      { color: "#FF492C", percent: 10 },
      { color: "#F6D438", percent: 16 },
      { color: "#DDD539", percent: 24 },
      { color: "#5ADD41", percent: 32 },
      { color: "#43A6F9", percent: 70 },
      { color: "#4786FF", percent: 80 },
      { color: "#483AEB", percent: 90 },
      { color: "#6300C7", percent: 100 }
    ];
          ...
          >
            ...
            >
              <m.path
                ...
                d={linePath}
                opacity={1}
                stroke={`url(#outside-temp-${timestampId})`}
                strokeOpacity="0.2"
                fill="none"
                strokeWidth={24}
                strokeLinecap="round"
